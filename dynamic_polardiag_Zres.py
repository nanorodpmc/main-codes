...
def select_data(val):
    ...
    for file in tqdm(files):
        ...
        z=int(file[:2])
        data[str(z)+str(angle)]=datum[1][1:-1]-min(datum[1][1:-1])
...
def polardiag(val):
    ...
    if value%2==0 :
        ...
        y=np.array([data[str(int(z_slider.val))+str(i)][point_slider.val] for i in range(int(max_angle)+1)]) #intensities for polar diagrams
...
def changespectrum(val):
    global z
    spectrum.set_ydata(data[str(z_slider.val)+str(angle_slider.val)])
    ax2.set_ylim(ymin=0,ymax=1.05*max(data[str(int(z_slider.val))+str(angle_slider.val)]))
    fig.canvas.draw_idle()
...
axangle = plt.axes([0.82, 0.65, 0.04, 0.35])
...
axz = plt.axes([0.92, 0.65, 0.04, 0.35])
